...
if($candidate->checkEmailExist($email))
{
  ...
  $query = $db->prepare("INSERT INTO `candidate_tbl`(`c_name`, `c_surname`, `c_email`, `c_cellphone`, `c_DOB`, `c_password`, `c_verified`, `popia_consent`, `t_and_c`, `date_registered`,`added_by`) VALUES ('$name','$surname','$email','$userCell','$dateOB','$password','$status','Accepted','Accepted','$date_registered','System')");
$query->execute();
$enemail = md5($email);
$pass = $pass;
