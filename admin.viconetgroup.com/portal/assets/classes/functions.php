...
class Candidates
{
	...
	private $currentJobCompany;

	private $prevJobTitle;
	private $prevJobCompany;
	private $prevJobEndDate;
	...
	public function trackEmailSentout($email, $emailType,$numReminder)
	{
		$c_email = $email;
		$email_type = $emailType;
		//$date = date('Y-m-d');
		$num_reminder = $numReminder;
		$sqlQuery = $this->db->prepare("INSERT INTO `reminders_email_tracking`(`c_email`, `email_type`, `num_reminder`) VALUES(?,?,?)");
...
INNER JOIN curriculum_vitae cv ON MD5(c.c_email) = cv.c_email
WHERE (cv.cv_file = '' OR cv.cv_file IS NULL)
 AND c.added_by != 'import' and c_verified not in ('Pending','process','google-process')");
		$query->execute();
		$row = $query->fetch();
		return $row['total_count'];
	}
...
");
 ...
 FROM candidate_tbl c 
 inner JOIN curriculum_vitae cv ON md5(c.c_email) = cv.c_email 
 WHERE COALESCE(cv.cv_file, '') != '' AND c.added_by != 'Import' and c_verified not in ('Pending','process','google-process')");
		$query->execute();
		$row = $query->fetch();
		return $row['total_count'];
	}
...
inner join curriculum_vitae cv on md5(c.c_email) = cv.c_email  where COALESCE(cv.cv_file, '') != '' and c.num_reminder != 0 ");
		$query->execute();
		$row = $query->fetch();
		return $row['total_count'];
	}
...
inner join curriculum_vitae cv on md5(c.c_email) = cv.c_email  where cv.cv_file != '' and c.num_reminder = ?");
	$query->execute(array($num_reminder));*/
		...
		$sql = "
        SELECT c.c_name,c.c_surname,c.c_email, c.c_DOB, c.c_cellphone,cv.date_cv_uploaded as date_uploaded,es.date_sent as date_sent
        FROM candidate_tbl c
        INNER JOIN curriculum_vitae cv ON MD5(c.c_email) = cv.c_email
		...
		if ($type === 'uploaded') {
			$sql .= "AND (COALESCE(cv.cv_file, '') != '' and es.num_reminder = (select max(sec_es.num_reminder) from reminders_email_tracking sec_es where sec_es.c_email = c.c_email and sec_es.email_type = '$emailType'))";
		} elseif ($type === 'notuploaded') {
			$sql .= " AND ((cv.cv_file ='' OR cv.cv_file is null) OR es.num_reminder < (select max(sec_es.num_reminder) from reminders_email_tracking sec_es where sec_es.c_email = c.c_email and sec_es.email_type = '$emailType'))";
		} 

		if ($emailNum !== 0) {
			$sql .= " AND es.num_reminder = :emailNum";
		}
...
inner join curriculum_vitae cv on md5(c.c_email) = cv.c_email  where cv.cv_file != '' and c.num_reminder != 0 GROUP BY c.num_reminder;");
	...
	public function getFilteredReminderData($emailNum, $fromDate, $toDate)
	{
		...
		$sql = "
        SELECT es.num_reminder, COUNT(es.num_reminder) AS num_reminds
        FROM candidate_tbl c
        INNER JOIN curriculum_vitae cv ON MD5(c.c_email) = cv.c_email
        INNER JOIN reminders_email_tracking es ON c.c_email = es.c_email
        WHERE es.email_type = '$emailType'
...
// Build the SQL query
$sql = "SELECT COUNT(c.c_email) AS num_rows 
        FROM candidate_tbl c
        INNER JOIN curriculum_vitae cv ON md5(c.c_email) = cv.c_email
        INNER JOIN reminders_email_tracking ret ON ret.c_email = c.c_email
        WHERE ret.email_type = :emailType";

// Condition based on $emailNum
if ($emailNum) {
    $sql .= " AND ret.num_reminder = :emailNum";
}

// Add date range condition if both dates are provided
if (!empty($fromDate) && !empty($toDate)) {
    $sql .= " AND ret.date_sent BETWEEN :fromDate AND :toDate";
}
...
return $result['num_rows'];
	...
	public function getNotUploaded($emailNum,$fromDate,$toDate)
	{
		$emailType = 'CV upload reminder';
		$sql = "SELECT count(c.c_email) as num_rows FROM candidate_tbl c
	inner join curriculum_vitae cv on md5(c.c_email) = cv.c_email
	inner join reminders_email_tracking es ON es.c_email = c.c_email
	where ((cv.cv_file = '' OR cv.cv_file is null) OR es.num_reminder < (select max(sec_es.num_reminder) from reminders_email_tracking sec_es where sec_es.c_email = c.c_email and sec_es.email_type = :emailType)) and es.num_reminder = :emailNum and email_type = :emailType";
		...
		$stmt->bindParam(':emailNum', $emailNum);
		$stmt->bindParam(':emailNum', $emailNum);
		//$stmt->bindParam(':toDate', $);
		$stmt->execute();
		$result = $stmt->fetch();

		return $result['num_rows'];
	}
	public function getUploaded($emailNum,$fromDate,$toDate)
	{
		$emailType = 'CV upload reminder';
		$sql = "SELECT count(c.c_email) as num_rows FROM candidate_tbl c
	inner join curriculum_vitae cv on md5(c.c_email) = cv.c_email
	inner join reminders_email_tracking es ON es.c_email = c.c_email
	where COALESCE(cv.cv_file, '') != ''  and es.num_reminder = :emailNum and es.num_reminder >= (select max(sec_es.num_reminder) from reminders_email_tracking sec_es where sec_es.c_email = c.c_email and sec_es.email_type = :emailType)  and email_type = :emailType";
	...
	public function candidateAllData()
	{
		$query = $this->db->prepare("SELECT c.c_name,c.c_surname,c.c_DOB,c.race, c.gender, c.c_email as email, c.c_cellphone, c.date_registered,c.c_verified, c.added_by, cv.bio, cv.years_experience, cv.work_method as work_methodd, cv.cv_file FROM candidate_tbl c
		inner join curriculum_vitae cv on md5(c.c_email)=cv.c_email
	...
	public function getCurrentJob($email)
{
    // Prepare the SQL query to select current job details based on email
    $query = $this->db->prepare("SELECT job_title, company_name FROM candidate_role WHERE c_email=? AND status='current'");
    
    // Execute the query with the provided email
    $query->execute(array($email));
    
    // Fetch the results and set the properties if a record is found
    $row = $query->fetch();
    if ($row) {
        $this->currentJobTitle = $row['job_title'];
        $this->currentJobCompany = $row['company_name'];
    } else {
        // Reset properties if no current job is found
        $this->currentJobTitle = "N/A";
        $this->currentJobCompany = "N/A";
    }
}

	public function checkCandidateExistInRoles($email){
		$query = $this->db->prepare("SELECT c_email FROM key_roles where c_email = ?");
		$query->execute(array($email));
		return $query;
	}
	public function getCurrentJobRoles($email){
		$jobRole = "";
		if ($this->checkCandidateExistInRoles($email)){
		$query = $this->db->prepare("SELECT GROUP_CONCAT(role SEPARATOR ', ') AS roles FROM key_roles WHERE c_email = ?");
		$query->execute(array($email));
		// Fetch the result
	...
	$jobRole = "";
	$query = $this->db->prepare("SELECT GROUP_CONCAT(skill SEPARATOR ', ') AS skills FROM key_skills WHERE c_email = ?");
	$query->execute(array($email));
		// Fetch the result
    $row = $query->fetch(PDO::FETCH_ASSOC);
    if ($row) {
	...
	//The the latest previous Job
	public function getLatestPreviousJob($email)
	{
		$query = $this->db->prepare("SELECT * FROM candidate_role WHERE c_email=? and status='post' and end_date=(SELECT max(end_date) from candidate_role where c_email = ?)");
		$query->execute(array($email,$email));
		 
		$rows = $query->fetch();
		if($rows){
		 	$this->prevJobTitle = $rows['job_title'];
		 	$this->prevJobCompany = $rows['company_name'];
		 	$this->prevJobEndDate = $rows['end_date'];
		 }
		 else{
		 	$this->prevJobTitle = "N/A";
		 	$this->prevJobCompany = "N/A";
		 	$this->prevJobEndDate = "N/A";
		 }
	}
	//The the latest previous Job
	public function getLatestQualification($email)
	{
		$query = $this->db->prepare("SELECT * FROM qualifications WHERE c_email=? and qw_date_completed=(SELECT max(qw_date_completed) from qualifications where c_email = ?)");
		$query->execute(array($email,$email));
		 
		$rows = $query->fetch();
		if($rows){
		 	$this->qualification = $rows['q_name'];
			$this->yearCompleted = $rows['qw_date_completed'];
			$this->fieldOfStudy = $rows['field_of_study'];
			$this->institution = $rows['institution_name'];
		 }
		 else{
		 	$this->qualification = "N/A";
			$this->yearCompleted = "N/A";
			$this->fieldOfStudy = "N/A";
			$this->institution = "N/A";
		 }
	}
	//Getting Qualification data
	...
	public function getPrevCompName(){
		return $this->prevJobCompany;
	}
	public function getPrevJobTitle(){
		return $this->prevJobTitle;
	}
	public function getPrevJobEndDate(){
		return $this->prevJobEndDate;
	}
}
/**
 * 
