...
if($check1 && $check2 && $check3 && $check4) {
    ...
    $amount = $cartTotal;
    $date = date('d-m-Y');
    $endDate = $pfData['custom_str1'];
    $numUsers = $pfData['custom_str2'];
    $inv_id = $pfData['m_payment_id'];
    $corp->compPackage($pack_id); 
    $item_desc = $corp->getPackDesc();
    if($pfData['custom_str3']=="Recurring")
    {
       $auto_renewal = "on";
       $corp->updateSubscription($pack_id,$status,$email_address,$date,$endDate,$auto_renewal);
       $corp->getAddressBy($encrEmail);
       include "send_invoice.php";
        $corp->updateUserSubscription($email_address,$date,$endDate);
        $corp->updateNumUsers($email_address,$numUsers);
        $corp->addInvoice($inv_id, $email_address, $corp->getPackPrice2(), $date, $status);
        /*if($corp->countInvPerComp($encrEmail) == 0)
        {
    ...
    {   
        $auto_renewal = "off";
        $corp->updateSubscription($pack_id,$status,$email_address,$date,$endDate,$auto_renewal);//This function changes the package from current to choosen package
        $corp->getAddressBy($encrEmail);
        include "send_invoice.php";
        $corp->updateUserSubscription($email_address,$date,$endDate); 
        $corp->updateNumUsers($email_address,$numUsers);//updates number of added users
        $corp->addInvoice($inv_id, $email_address, $corp->getPackPrice2(), $date, $status); 
    }
